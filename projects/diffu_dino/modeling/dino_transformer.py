...
from .bbox_embedd import TimeStepBlock
    ...
    def __init__(
        ...
        super(DINOTransformerDecoder, self).__init__(
            transformer_layers=BaseTransformerLayer(
                ...
                time_step_embed_query=TimeStepBlock(
                    channels =  embed_dim ,
...
class DINOTransformer(nn.Module):
    ...
    def __init__(
        ...
        ## Diffu_dino
        self.tgt_embed_new = tgt_embed_new
        self.cdn_with_timestep = cdn_with_timestep
        
        if self.learnt_init_query:
            if self.tgt_embed_new:
                self.tgt_embed = nn.Embedding(1, self.embed_dim)
            else:
                self.tgt_embed = nn.Embedding(self.two_stage_num_proposals, self.embed_dim)
        time_embed_dim = self.embed_dim * 4
        self.time_embed = nn.Sequential(
            linear(self.embed_dim, time_embed_dim),
            nn.SiLU(),
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.decoder.class_embed is None:
            enc_outputs_class_emb= self.decoder.class_embed1[self.decoder.num_layers](output_memory)
            enc_outputs_class = self.decoder.class_embed2[self.decoder.num_layers](enc_outputs_class_emb)
        else:
        ...
        ## Diffu_dino
        if not is_train:
            return memory , mask_flatten , spatial_shapes , level_start_index, valid_ratios , reference_points, target_unact.detach()
        ...
        if self.learnt_init_query:
            if self.tgt_embed_new:
                ...
                # target = self.tgt_embed.weight[None].repeat(bs, num_queries, 1)##tgt_embed
                target = self.tgt_embed.weight[None].repeat(bs, self.two_stage_num_proposals, 1) ##num_queries
            else:
                # print("in old tgt_embed")
                target = self.tgt_embed.weight[None].repeat(bs, 1, 1)
        ...
        ## Diffu_dino
        time_steps = timestep_embedding(time_steps.float() , self.embed_dim, repeat_only=False)
